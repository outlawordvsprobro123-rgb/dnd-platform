    ...
    <div style={{ height: '100vh', background: 'var(--bg-base)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        <main style={{ flex: 1, overflow: 'hidden', position: 'relative', background: '#0a0805' }}>
          ...
          {/* Индикатор realtime */}
          <div style={{ position: 'absolute', top: '0.75rem', left: '0.75rem', zIndex: 20, display: 'flex', alignItems: 'center', gap: '0.4rem', background: 'rgba(0,0,0,0.75)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '9999px', padding: '0.2rem 0.7rem' }}>
            <span style={{
              width: 6, height: 6, borderRadius: '50%', flexShrink: 0,
              background: realtimeStatus === 'connected' ? '#4ade80' : realtimeStatus === 'polling' ? '#fb923c' : realtimeStatus === 'error' ? '#f87171' : '#facc15',
            }} />
            <span style={{ fontSize: '0.6rem', color: '#9ca3af', fontFamily: 'monospace' }}>
              {realtimeStatus === 'connected' ? 'live' : realtimeStatus === 'polling' ? 'poll 2s' : realtimeStatus === 'error' ? 'err' : '...'}
            </span>
          </div>
